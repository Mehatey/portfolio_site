...
const list = [
  ...
  { slug: "naavo", asset: "../round-3/naavo-gift.webp", after: "7.naavo/15.1.webp", extra: "../round-3/naavo-use.webp" },
  { slug: "illustrations", asset: "apna.webp", after: "11.illu/30.1.webp" },
  { slug: "shot-on-iphone", asset: "shot.webp", extra: "billboard.webp", after: "8.shotoniphone/3.webp" },
];
...
const figure = (asset) =>
  `<figure class="review-insert" id="new-image${asset === "billboard.webp" || asset === "naavo-use.webp" ? "-2" : asset === "naavo-incense.webp" ? "-3" : ""}"><img src="${root}round-4/${asset}" alt="Proposed illustrative mockup" loading="eager"><figcaption>Illustrative mockup · placement preview</figcaption></figure>`;
for (const p of list) {
  ...
  if (p.hero) html = html.replaceAll('src="/_site/' + p.hero + '"', 'src="' + root + "round-4/" + p.asset + '"');
  ...
  if (p.slug === "naavo") {
    ...
    if (incAt > 0) html = html.slice(0, incClose) + figure("../round-3/naavo-incense.webp") + html.slice(incClose);
  }
  html = html.replace(
    "</head>",
