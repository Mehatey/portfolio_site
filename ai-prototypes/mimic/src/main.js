...
// ---------- boot guards ----------
if (!navigator.gpu) {
  els.introErr.textContent = "This live version needs WebGPU. Open it in a current desktop Chrome, Edge, or Safari 18+ browser.";
  ...
  addEventListener("pointerdown", (e) => {
    if (
      e.target.closest("#sound") ||
      e.target.closest("#motion") ||
      e.target.closest("#recenter") ||
  ...
  function focusPick(node) {
    ...
    if (node.kind === "ghost") {
      ...
      const sound = node.meta.sound ? Promise.resolve(node.meta.sound) : fetchTaxonSound(node.meta.taxonId);
      sound.then((url) => {
  ...
  function showDetail(node) {
    ...
    if (node.kind === "ghost") {
      ...
      d.innerHTML =
        (m.photo
          ? `<img class="photo" src="${esc(m.photo)}" alt="${esc(m.common || m.sci || "animal observation")}" referrerpolicy="no-referrer" />`
          : "") +
        `<div class="body">` +
        ...
        (m.observationId
          ? `<a class="source" href="https://www.inaturalist.org/observations/${encodeURIComponent(m.observationId)}" target="_blank" rel="noopener noreferrer">view source on iNaturalist ↗</a>`
          : "") +
  ...
  els.retry.addEventListener("click", (e) => {
    e.stopPropagation();
    location.reload();
  });
