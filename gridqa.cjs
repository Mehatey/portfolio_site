    ...
    try {
      ...
      const o = await p.evaluate(() => {
        ...
        document.querySelectorAll(".cs-grid, .cs-grid-3, .cs-pair").forEach((g) => {
          ...
          const tops = med.map((m) => m.getBoundingClientRect().top),
            bots = med.map((m) => m.getBoundingClientRect().bottom);
          const dt = Math.max(...tops) - Math.min(...tops),
            db = Math.max(...bots) - Math.min(...bots);
          ...
          if (d > 3) {
            ...
            if (d > worst) {
              worst = d;
              ex =
                (med[0].currentSrc || med[0].src || "").split("/").pop() +
                " vs " +
                (med[1].currentSrc || med[1].src || "").split("/").pop() +
                " Δtop=" +
                dt.toFixed(0) +
                " Δbot=" +
                db.toFixed(0);
        ...
        document.querySelectorAll(".cs-intro, .cs-section > h2, .cube-cap, .proj-highlights, .project-details").forEach((e) => {
          ...
          if (x.width > 100) lefts.add(Math.round(x.left));
        });
        ...
        document.querySelectorAll(".case-story img, .case-story video, .cs-bleed img, .cs-bleed video").forEach((m) => {
          ...
          if (rr.width < 80 || rr.height < 80) return;
          const nw = m.naturalWidth || m.videoWidth,
            nh = m.naturalHeight || m.videoHeight;
          if (!nw || !nh) return;
          const f = getComputedStyle(m).objectFit,
            ba = rr.width / rr.height,
            na = nw / nh;
          if (Math.abs(ba - na) / na > 0.09) {
            if (f === "cover") crop++;
            else if (f === "contain") let_++;
          }
        });
      ...
      say(r.padEnd(24) + "FAILED " + String(e).slice(0, 60));
    }
  }
  await b.close();
})();
