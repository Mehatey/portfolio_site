...
const sun = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({ color: 0xf09a45 }),
);
...
function limb(parent, length, radius, mat) {
  const m = mesh(
    new THREE.CapsuleGeometry(radius, length - radius * 2, 8, 20),
    mat,
    parent,
...
renderer.domElement.addEventListener("pointerdown", (e) => {
  ...
  if (ray.intersectObjects(pickables, false).length)
    gsap.fromTo(
      state,
      { click: 1 },
  ...
  scrollTrigger: {
    ...
    onUpdate: (s) => {
      ...
      document.querySelector(".chapter-no").textContent =
        `${String(Math.min(4, Math.floor(s.progress * 4) + 1)).padStart(2, "0")} / 04`;
...
tl.to(
  ...
  .to(
    monolith.rotation,
    { y: Math.PI * 1.3, z: 0.14, duration: 1, ease: "none" },
    1,
  )
  .to(
    monolith.position,
...
function smoothWindow(p, a, b, c, d) {
  return (
    ...
    (1 - THREE.MathUtils.smoothstep(p, c, d))
  );
}
function pathX(p) {
...
function animate() {
  ...
  kavi.position.set(
    pathX(p),
    -crouch + jump,
    0.15 + Math.sin(p * Math.PI) * 0.1,
  );
  kavi.rotation.y = THREE.MathUtils.lerp(
    kavi.rotation.y,
    (p < 0.33 ? -1 : p < 0.66 ? 1 : -1) * 0.14,
    0.06,
  );
  hips.position.y =
    Math.abs(Math.cos(phase)) * walk * 0.055 + Math.sin(t * 1.5) * 0.018;
  ...
  headPivot.rotation.x = THREE.MathUtils.lerp(
    ...
    -pointer.y * 0.2 + state.click * 0.12,
    0.07,
  );
  pupils.forEach((o) => {
    o.mesh.position.x = THREE.MathUtils.lerp(
      o.mesh.position.x,
      o.base + pointer.x * 0.035,
      0.12,
    );
    o.mesh.position.y = THREE.MathUtils.lerp(
      o.mesh.position.y,
      0.04 + pointer.y * 0.03,
      0.12,
    );
  ...
  camera.position.x = THREE.MathUtils.lerp(
    camera.position.x,
    shot.x + pointer.x * 0.09,
    0.06,
  );
  camera.position.y = THREE.MathUtils.lerp(
    camera.position.y,
    shot.y + pointer.y * 0.05,
    0.06,
  );
