...
const PALETTES = {
  indigo:  [[0.78,0.58,0.16],[0.92,0.20,0.04],[0.08,0.74,0.44],[0.06,0.26,0.86]],
  ink:     [[0.72,0.66,0.58],[0.40,0.44,0.52],[0.16,0.20,0.30]],
  bloom:   [[0.10,0.72,0.52],[0.20,0.34,0.80],[0.86,0.30,0.10]],
  forest:  [[0.30,0.10,0.60],[0.62,0.22,0.52],[0.14,0.46,0.72]],
};
...
const DEFAULTS = {
  ...
  hover:      0.22,       /* subtle cursor wake; 0 disables hover motion      */
...
function Watercolour(canvas, opts){
  ...
  const prog=(f)=>{const p=gl.createProgram();gl.attachShader(p,sh(gl.VERTEX_SHADER,VS));
    ...
    for(let i=0;i<n;i++){const nm=gl.getActiveUniform(p,i).name.replace(/\[0\]$/,'');
      u[nm]=gl.getUniformLocation(p,nm);} return {p,u};};

  const P={advv:prog(F_ADVV),force:prog(F_FORCE),div:prog(F_DIV),jac:prog(F_JAC),
           grad:prog(F_GRAD),advp:prog(F_ADVP),drop:prog(F_DROP),paint:prog(F_PAINT)};
  ...
  let W=0,H=0,SW=0,SH=0,vel,velF,pig,pigF,prs,prsF,dvT,dvF,cur=0,pc=0,pg=0,alive=true;
  function mk(w,h,i,f,t,filt){const x=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,x);
  ...
  function alloc(){
    ...
    SW=Math.max(48,Math.floor(W*O.scale)); SH=Math.max(48,Math.floor(H*O.scale));
    const RG=HF?gl.RG16F:gl.RGBA, T=HF?gl.HALF_FLOAT:gl.UNSIGNED_BYTE, RGBA=HF?gl.RGBA16F:gl.RGBA;
    vel=[mk(SW,SH,RG,gl.RG,T),mk(SW,SH,RG,gl.RG,T)]; velF=vel.map(fb);
    pig=[mk(SW,SH,RGBA,gl.RGBA,T,gl.LINEAR_MIPMAP_LINEAR),
         mk(SW,SH,RGBA,gl.RGBA,T,gl.LINEAR_MIPMAP_LINEAR)]; pigF=pig.map(fb);
    prs=[mk(SW,SH,RG,gl.RG,T),mk(SW,SH,RG,gl.RG,T)]; prsF=prs.map(fb);
    dvT=mk(SW,SH,RG,gl.RG,T); dvF=fb(dvT);
    [].concat(velF,pigF,prsF).forEach(f=>{gl.bindFramebuffer(gl.FRAMEBUFFER,f);
      gl.clearColor(0,0,0,0);gl.clear(gl.COLOR_BUFFER_BIT);});
    gl.bindFramebuffer(gl.FRAMEBUFFER,null);
  }
  alloc();
  const ro = ('ResizeObserver' in window) ? new ResizeObserver(()=>alloc()) : null;
  if (ro) ro.observe(canvas); else window.addEventListener('resize',alloc);

  const use=(f,w,h,pr)=>{gl.bindFramebuffer(gl.FRAMEBUFFER,f);gl.viewport(0,0,w,h);
    gl.useProgram(pr.p);gl.bindVertexArray(vao);};
  const bind=(u,i,t)=>{gl.activeTexture(gl.TEXTURE0+i);gl.bindTexture(gl.TEXTURE_2D,t);gl.uniform1i(u,i);};
  const go=()=>gl.drawArrays(gl.TRIANGLES,0,3);

  const queue=[];
  function drop(x,y,ink,r,amt){ queue.push({x,y,ink:ink||INKS[(Math.random()*INKS.length)|0],
    r:r||0.06+Math.random()*0.05, a:amt||0.55, t:0}); }
  const forces=[];
  function push(x,y,dx,dy,r,amt){ forces.push({x,y,dx,dy,r:r||0.1,a:amt||1}); }

  /* pointer */
  let mx=.5,my=.5,px=.5,py=.5,down=false,lx=0,ly=0,moved=false;
  function rel(e){const b=canvas.getBoundingClientRect();
    return [(e.clientX-b.left)/b.width, 1-(e.clientY-b.top)/b.height];}
  const onMove=e=>{const p=rel(e),vx=p[0]-mx,vy=p[1]-my;
    if(!down&&O.hover>0&&Math.hypot(vx,vy)>.0003)
      forces.push({x:p[0],y:p[1],dx:vx*18,dy:vy*18,r:.13,a:O.hover});
    mx=p[0];my=p[1];
    if(down){const dx=e.clientX-lx,dy=e.clientY-ly;
      if(!moved&&Math.hypot(dx,dy)>4)moved=true; lx=e.clientX;ly=e.clientY;}};
  const onDown=e=>{down=true;moved=false;lx=e.clientX;ly=e.clientY;};
  const onUp=e=>{const p=rel(e); if(down&&!moved) drop(p[0],p[1]); down=false;};
  if(O.pointer){ window.addEventListener('pointermove',onMove);
    canvas.addEventListener('pointerdown',onDown); window.addEventListener('pointerup',onUp); }

  let t0=performance.now(),T=0,nextDrop=0.6,raf=0;
  function frame(now){
    if(!alive) return;
    raf=requestAnimationFrame(frame);
    const dt=Math.max(.001,Math.min(.05,(now-t0)/1000)); t0=now; T=now*.001;
    const AR=[Math.max(1,W/H),Math.max(1,H/W)], tx=[1/SW,1/SH];
    const mvx=mx-px,mvy=my-py; px=mx; py=my;

    /* advect velocity */
    use(velF[1-cur],SW,SH,P.advv); bind(P.advv.u.uVel,0,vel[cur]);
    gl.uniform2f(P.advv.u.uTexel,tx[0],tx[1]); gl.uniform1f(P.advv.u.uDt,dt);
    gl.uniform1f(P.advv.u.uTime,T); gl.uniform1f(P.advv.u.uDrift,O.ambient);
    gl.uniform1f(P.advv.u.uDamp,.985); go(); cur=1-cur;

    /* pointer drag, plus anything push() queued */
    if(O.pointer && down && Math.hypot(mvx,mvy)>.0004)
      forces.push({x:mx,y:my,dx:mvx*38,dy:mvy*38,r:.10,a:1});
    while(forces.length){ const f=forces.shift();
      use(velF[1-cur],SW,SH,P.force); bind(P.force.u.uVel,0,vel[cur]);
      gl.uniform2f(P.force.u.uP,f.x,f.y); gl.uniform2f(P.force.u.uD,f.dx,f.dy);
      gl.uniform2f(P.force.u.uAspect,AR[0],AR[1]);
      gl.uniform1f(P.force.u.uR,f.r); gl.uniform1f(P.force.u.uAmt,f.a); go(); cur=1-cur; }

    /* project to divergence-free so pigment spreads instead of piling */
    use(dvF,SW,SH,P.div); bind(P.div.u.uVel,0,vel[cur]);
    gl.uniform2f(P.div.u.uTexel,tx[0],tx[1]); go();
    gl.bindFramebuffer(gl.FRAMEBUFFER,prsF[0]); gl.clearColor(0,0,0,0); gl.clear(gl.COLOR_BUFFER_BIT);
    for(let i=0;i<O.iterations;i++){
      use(prsF[1-pc],SW,SH,P.jac); bind(P.jac.u.uP,0,prs[pc]); bind(P.jac.u.uDiv,1,dvT);
      gl.uniform2f(P.jac.u.uTexel,tx[0],tx[1]); go(); pc=1-pc; }
    use(velF[1-cur],SW,SH,P.grad); bind(P.grad.u.uVel,0,vel[cur]); bind(P.grad.u.uP,1,prs[pc]);
    gl.uniform2f(P.grad.u.uTexel,tx[0],tx[1]); go(); cur=1-cur;

    /* carry the pigment */
    use(pigF[1-pg],SW,SH,P.advp); bind(P.advp.u.uPig,0,pig[pg]); bind(P.advp.u.uVel,1,vel[cur]);
    gl.uniform2f(P.advp.u.uTexel,tx[0],tx[1]); gl.uniform1f(P.advp.u.uDt,dt);
    gl.uniform1f(P.advp.u.uDiff,O.diffuse); gl.uniform1f(P.advp.u.uDry,O.dry);
    gl.uniform1f(P.advp.u.uSettle,O.settle); go(); pg=1-pg;

    /* pigment falling on its own, so it is alive with nobody touching it */
    if(O.drops>0){ nextDrop-=dt*O.drops;
      if(nextDrop<=0){ nextDrop=1.4+Math.random()*3.4;
        drop(.12+Math.random()*.76, .12+Math.random()*.76); } }

    for(let i=queue.length-1;i>=0;i--){ const d=queue[i];
      use(pigF[1-pg],SW,SH,P.drop); bind(P.drop.u.uPig,0,pig[pg]);
      gl.uniform2f(P.drop.u.uP,d.x,d.y); gl.uniform2f(P.drop.u.uAspect,AR[0],AR[1]);
      gl.uniform3fv(P.drop.u.uInk,d.ink); gl.uniform1f(P.drop.u.uR,d.r);
      gl.uniform1f(P.drop.u.uAmt,d.a*(1-d.t)); gl.uniform1f(P.drop.u.uWet,1);
      go(); pg=1-pg; d.t+=dt*3.4; if(d.t>=1) queue.splice(i,1); }

    /* the sheet */
    gl.bindFramebuffer(gl.FRAMEBUFFER,null); gl.viewport(0,0,W,H);
    gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D,pig[pg]); gl.generateMipmap(gl.TEXTURE_2D);
    gl.useProgram(P.paint.p); gl.bindVertexArray(vao);
    bind(P.paint.u.uPig,0,pig[pg]);
    gl.uniform2f(P.paint.u.uRes,W,H); gl.uniform2f(P.paint.u.uTexel,1/W,1/H);
    gl.uniform1f(P.paint.u.uTime,T); gl.uniform3fv(P.paint.u.uPaper,O.paper);
    gl.uniform1f(P.paint.u.uGrain,O.grain); gl.uniform1f(P.paint.u.uEdge,O.edge);
    gl.uniform1f(P.paint.u.uGran,O.granulate); gl.uniform1f(P.paint.u.uVig,O.vignette);
    gl.uniform1f(P.paint.u.uBloom,O.bloom); gl.uniform1f(P.paint.u.uDens,O.density);
    go(); gl.bindVertexArray(null);
  }
  raf=requestAnimationFrame(frame);
  ...
  return {
    ...
    destroy(){ alive=false; cancelAnimationFrame(raf);
      if(ro) ro.disconnect(); else window.removeEventListener('resize',alloc);
      if(O.pointer){ window.removeEventListener('pointermove',onMove);
        canvas.removeEventListener('pointerdown',onDown); window.removeEventListener('pointerup',onUp); } }
  };
}
