...
class SceneOptimizer {
  ...
  _createBatchedMeshes(batchGroups) {
    ...
    for (const [, group] of batchGroups) {
      ...
      const maxVertices = Array.from(group.geometryStats.values()).reduce((sum, stats) => sum + stats.vertices, 0);
      const maxIndices = Array.from(group.geometryStats.values()).reduce((sum, stats) => sum + stats.indices, 0);
  ...
  removeEmptyNodes(object) {
    ...
    for (const child of children) {
      ...
      if ((child instanceof THREE.Group || child.constructor === THREE.Object3D) && child.children.length === 0) {
        object.remove(child);
      }
    }
  }
