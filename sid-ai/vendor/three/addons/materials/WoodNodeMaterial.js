...
const noiseFbm = TSL.Fn(([p, detail, roughness, lacunarity, useNormalize]) => {
  ...
  return TSL.select(
    ...
    TSL.select(useNormalize.equal(1), sum.div(maxamp).mul(0.5).add(0.5), sum)
  );
});
...
const spaceWarp = TSL.Fn(([p, warpStrength, xyScale, zScale]) => {
  ...
  const noise = noiseFbm3d(combinedXyz.mul(1.6 * 1.5), TSL.float(1), TSL.float(0.5), TSL.float(2), TSL.int(1))
    .sub(0.5)
    .mul(warpStrength);
...
const woodRings = TSL.Fn(([w, ringThickness, ringBias, ringSizeVariance, ringVarianceScale, barkThickness]) => {
  const rings = noiseFbm(w.mul(ringVarianceScale), TSL.float(1), TSL.float(0.5), TSL.float(1), TSL.int(1))
...
uniforms.darkGrainColor = TSL.uniform(new THREE.Color(params.darkGrainColor)).onObjectUpdate(({ material }, self) =>
  self.value.set(material.darkGrainColor)
);
uniforms.lightGrainColor = TSL.uniform(new THREE.Color(params.lightGrainColor)).onObjectUpdate(({ material }, self) =>
  self.value.set(material.lightGrainColor)
);
uniforms.transformationMatrix = TSL.uniform(new THREE.Matrix4().copy(params.transformationMatrix)).onObjectUpdate(
  ({ material }) => material.transformationMatrix
);
