...
class SVGLoader extends Loader {
  ...
  static createShapes(shapePath) {
    ...
    function getIntersections(path1, path2) {
      ...
      for (let index = 1; index < path1.length; index++) {
        ...
        for (let index2 = 1; index2 < path2.length; index2++) {
          ...
          if (
            intersection !== null &&
            intersectionsRaw.find((i) => i.t <= intersection.t + Number.EPSILON && i.t >= intersection.t - Number.EPSILON) === undefined
          ) {
    ...
    const isAHole = simplePaths.map((p) =>
      isHoleTo(p, simplePaths, scanlineMinX, scanlineMaxX, shapePath.userData ? shapePath.userData.style.fillRule : undefined)
    );
