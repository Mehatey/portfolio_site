    ...
    Promise.all([getImageDataFromBuffer(sdrBuffer), getImageDataFromBuffer(gainmapBuffer)])
      .then(([sdrImage, gainmapImage]) => {
        ...
        const unclampedWeightFactor =
          (Math.log2(maxDisplayBoost) - xmpMetadata.hdrCapacityMin) / (xmpMetadata.hdrCapacityMax - xmpMetadata.hdrCapacityMin);
        ...
        for (let pixelIndex = 0; pixelIndex < sdrImageData.data.length; pixelIndex += 4) {
          ...
          for (let channelIndex = 0; channelIndex < 3; channelIndex++) {
            ...
            const hdrValue =
              (sdrValue + xmpMetadata.offsetSDR) * (logBoost * weightFactor === 0.0 ? 1.0 : Math.pow(2, logBoost * weightFactor)) -
