...
class ChromaticAberrationNode extends TempNode {
  ...
  setup(/* builder */) {
    ...
    const chromaticAberrationFn = Fn(() => {
      return ApplyChromaticAberration(uvNode, this.strengthNode, this.centerNode, this.scaleNode, this._invSize);
    });
...
export const chromaticAberration = (node, strength = 1.0, center = null, scale = 1.1) => {
  return nodeObject(new ChromaticAberrationNode(convertToTexture(node), nodeObject(strength), nodeObject(center), nodeObject(scale)));
};
