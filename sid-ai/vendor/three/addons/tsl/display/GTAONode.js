        ...
        Loop({ end: STEPS, type: "int", name: "j", condition: "<" }, ({ j }) => {
          ...
          If(abs(viewDeltaX.z).lessThan(this.thickness), () => {
            ...
            cosHorizons.x.addAssign(
              max(0, mul(sampleCosHorizon.sub(cosHorizons.x), mix(1.0, float(2.0).div(float(j).add(2)), this.distanceFallOff)))
            );
          });
          ...
          If(abs(viewDeltaY.z).lessThan(this.thickness), () => {
            ...
            cosHorizons.y.addAssign(
              max(0, mul(sampleCosHorizon.sub(cosHorizons.y), mix(1.0, float(2.0).div(float(j).add(2)), this.distanceFallOff)))
            );
          });
        });
