  ...
  setup(builder) {
    ...
    const pointPlaneDistance = Fn(([point, planePoint, planeNormal]) => {
      ...
      const denominator = sqrt(
        mul(planeNormal.x, planeNormal.x).add(mul(planeNormal.y, planeNormal.y)).add(mul(planeNormal.z, planeNormal.z))
      ).toVar();
...
export const ssr = (colorNode, depthNode, normalNode, metalnessNode, roughnessNode = null, camera = null) =>
  nodeObject(
    new SSRNode(nodeObject(colorNode), nodeObject(depthNode), nodeObject(normalNode), nodeObject(metalnessNode), nodeObject(roughnessNode), camera)
  );
