...
(async()=>{
 ...
 for (const r of process.argv.slice(2)){
  const errs=[]; const h=e=>errs.push(String(e).slice(0,90)); p.on("pageerror",h);
  try{
   await p.goto("http://127.0.0.1:4000"+r,{waitUntil:"domcontentloaded",timeout:40000});
   await p.evaluate(()=>{document.querySelectorAll("img[loading=lazy]").forEach(i=>i.loading="eager")});
   await S(3000);
   await p.evaluate(()=>scrollTo(0,document.documentElement.scrollHeight*0.5)); await S(900);
   await p.evaluate(()=>scrollTo(0,0)); await S(500);
   const o=await p.evaluate(()=>{
    ...
    document.querySelectorAll(".cs-grid, .cs-grid-3").forEach(g=>{
      ...
      if(m.length<2) return; rows++;
      const t=m.map(x=>x.getBoundingClientRect().top), bt=m.map(x=>x.getBoundingClientRect().bottom);
      if(Math.max(Math.max(...t)-Math.min(...t), Math.max(...bt)-Math.min(...bt))>3) off++;
    });
    if(off) bad.push("GRID-MISALIGN "+off+"/"+rows);
    // rules over media
    const media=[...document.querySelectorAll("img,video")].map(m=>m.getBoundingClientRect()).filter(x=>x.width>200&&x.height>120);
    let hits=0;
    document.querySelectorAll("body *").forEach(e=>{const x=e.getBoundingClientRect(); if(x.height>3||x.height<0.5||x.width<220)return;
      const cs=getComputedStyle(e); if(cs.opacity==="0"||cs.visibility==="hidden"||cs.display==="none")return;
      if((cs.backgroundColor==="rgba(0, 0, 0, 0)"||cs.backgroundColor==="transparent")&&cs.borderTopWidth==="0px")return;
      for(const m of media) if(x.top>m.top+6&&x.bottom<m.bottom-6&&x.left<m.right-20&&x.right>m.left+20){hits++;break}});
  ...
  }catch(e){ say(r.padEnd(22)+"FAILED "+String(e).slice(0,50)); }
  p.off("pageerror",h);
 }
 await b.close();})();
