...
(function () {
  ...
  try {
    ...
    if (alreadySeen) {
      // returning within the session: start frozen, no repeat expand. Seek the
      // pixel clip to its final frame and grab it so the resting base shows the
      // real pixel image instead of an empty (black) texture.
      pixEnded = true;
      var grabStill = function () {
        try {
          vPix.currentTime = Math.max(0, (vPix.duration || 1) - 0.05);
        } catch (e) {
          captureStill();
        }
      };
      ...
      if (vPix.readyState >= 1) grabStill();
      else vPix.addEventListener("loadedmetadata", grabStill, { once: true });
    } else {
